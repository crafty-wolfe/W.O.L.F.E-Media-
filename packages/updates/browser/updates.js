...
function openUpdateManager() {
  const show = status => {
    ...
    document.getElementById('installUpdate')?.addEventListener('click', async () => { const button = document.getElementById('installUpdate'); button.disabled = true; button.textContent = 'Preparing…'; try { const response = await fetch('/api/updates/install', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'Unable to install the update.'); button.textContent = 'Restarting W.O.L.F.E…'; } catch (error) { button.disabled = false; button.textContent = 'Try again'; errorDialog(error); } });
  };
